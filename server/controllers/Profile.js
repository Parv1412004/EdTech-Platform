...
exports.gettingEnrolled = async (req,res) => {
  try {
    const userId = req.user.id;
    ...
    course.studentsEnrolled.push(new mongoose.Types.ObjectId(userId));
...
exports.isEnrolled = async (req,res) => {
  try {
    ...
    //console.log(course);
    const ispresent = course.studentsEnrolled.includes(new mongoose.Types.ObjectId(userId))
    if(ispresent){
      res.status(200).json({
        success:true,
        ispresent:true,
...
exports.courseProgress = async (req,res)=>{
  try {
    ...
    for(let i=0; i<course.courseContent.length; i++){
      totalsubsection+=course.courseContent[i].subSection.length
    }
    //console.log(totalsubsection);
    const courseprogress = await CourseProgress.findOne({courseID:courseId,userId:userId});
    //console.log(courseprogress)
    let completedSubSection = courseprogress.completedVideos.length

    res.status(200).json({
      success:true,
      progress: totalsubsection===0? 0 : Math.round(completedSubSection*100/totalsubsection),
    })
  } catch (error) {
    console.error(error)
    res.status(500).json({ message: "Server Error!" })
